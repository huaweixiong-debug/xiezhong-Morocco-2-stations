...
class Settings:
    ...
    ateq_slaves: tuple[int, int] = (255, 255)
    scanner_ip: str = "192.168.2.10"
    scanner_port: int = 9004
    database_host: str = "127.0.0.1"
    database_port: int = 3306
    credential_path: Path = Path(r"C:\ProgramData\LeakTest2Channels\db.json")
    ...
    def from_toml(cls, path: Path) -> "Settings":
        ...
        unknown = set(values) - {"mode", "plc_ip", "plc_poll_ms", "ateq_com_a", "ateq_com_b", "database",
                                 "ateq_slave_a", "ateq_slave_b", "scanner_ip", "scanner_port",
                                 "database_host", "database_port", "credential_path", "data_dir",
                                 "ports_confirmed"}
        if unknown:
        ...
        slaves = (int(values.get("ateq_slave_a", 255)), int(values.get("ateq_slave_b", 255)))
        if any(not 1 <= value <= 255 for value in slaves): raise ValueError("无效 ATEQ 从站地址")
        scanner_ip = str(values.get("scanner_ip", "192.168.2.10"))
        try: ipaddress.ip_address(scanner_ip)
        except ValueError as exc: raise ValueError("无效扫码枪 IP") from exc
        scanner_port = int(values.get("scanner_port", 9004)); database_port = int(values.get("database_port", 3306))
        if not 1 <= scanner_port <= 65535 or not 1 <= database_port <= 65535: raise ValueError("无效 TCP 端口")
        confirmed_raw = values.get("ports_confirmed", False)
        confirmed = confirmed_raw if isinstance(confirmed_raw, bool) else str(confirmed_raw).lower() == "true"
        return cls(mode=RunMode(mode), plc_ip=ip, plc_poll_ms=poll, ateq_ports=ports,
                   database=str(values.get("database", "test")), config_source=str(path),
                   ports_confirmed=confirmed, ateq_slaves=slaves, scanner_ip=scanner_ip,
                   scanner_port=scanner_port, database_host=str(values.get("database_host", "127.0.0.1")),
                   database_port=database_port, credential_path=Path(str(values.get("credential_path", r"C:\ProgramData\LeakTest2Channels\db.json"))),
