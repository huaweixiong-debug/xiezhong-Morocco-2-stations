...
验证流程与 A、B 工位分别运行，互不等待、互不共享状态。每个工位的校准倒计时到期后，系统保持对应的“校准到期”状态，直到操作员点击“启动验证”。启动后，单测/双测模式立即冻结，不能在验证中途切换。

PLC 验证请求点如下（置 1 表示持续请求，阶段结束后置 0）：

| 含义 | 工位 A | 工位 B | 置 1 时机 |
| --- | --- | --- | --- |
| 校准到期 | `M1.0` | `M1.1` | 倒计时到期，直到 OK 验证结果符合预期 |
| NG 样件请求 | `M1.2` | `M1.4` | 点击启动验证后，直到 NG 验证结果符合预期 |
| OK 样件请求 | `M1.3` | `M1.5` | NG 验证结果符合预期后，直到 OK 验证结果符合预期 |

模式选择点：A 工位 `M0.5`、B 工位 `M0.4`；单测为 `0`，双测为 `1`。验证样件和正常件使用同一套模式点，但验证开始后模式被冻结。

验证步骤：

1. **启动验证**：对应的校准到期点保持为 1，NG 样件请求点置 1；系统清除上一周期残留的扫码完成脉冲。
2. **NG 验证件测试**：
   - 单测：完成一次预期的正压 NG 结果。
   - 双测：完成一次预期的负压 NG 结果；该 NG 结果结束此样件，不再进行第二腔。
3. **NG 结果符合预期**：打印对应工位的 NG 验证标签；打印回执确认后才发送扫码枪 `LON`，NG 请求点置 0，OK 请求点置 1。
4. **扫码确认 NG 标签**：只有扫到当前打印标签的正确二维码才算确认；扫到其他无意义码会被忽略并继续等待。扫对后向 PLC 发对应工位的扫码完成脉冲（A=`M0.1`，B=`M0.0`，保持 2 秒后自动复位）。
5. **OK 验证件测试**：
   - 单测：完成一次预期的正压 OK 结果。
   - 双测：必须先得到负压 OK，再得到正压 OK；两腔都 OK 才算验证通过。
6. **OK 结果符合预期**：上位机收到预期 OK 结果后，将校准到期和 OK 请求点复位（`M1.0/M1.1`、`M1.3/M1.5`）；随后打印对应工位的 OK 验证标签，打印回执确认后才发送 `LON`。扫对 OK 标签后再次发出对应的 `M0.1/M0.0` 2 秒脉冲，验证流程才真正释放工位并恢复正常测试。

验证安全规则：结果不符合预期时不打印、不启动扫码枪、不发送扫码完成信号；打印失败时同样不启动扫码枪。复位会取消当前待扫码状态并关闭扫码枪，但不会伪造验证通过信号。

### 正常测试流程（生产件）

正常测试同样按 A、B 工位独立运行。校准验证未完成或工位处于故障恢复状态时，不允许开始新的生产周期。

1. **准备周期**：选择产品型号、人员和单测/双测模式。扫描当前工位的生产二维码后创建唯一 `cycle_id`，冻结产品、序列号、ATEQ 程序号、工位号和打印模板。A/B 可以同时准备和测试。
2. **第一次 ATEQ 测试**：PLC/ATEQ 完成第一阶段后写入第一阶段压力和泄漏结果。
   - 单测：结果为 NG 时记录 NG 并结束；结果为 OK 时进入打印。
   - 双测：第一阶段 NG 时记录 NG 并结束；第一阶段 OK 时进入第二阶段。
3. **第二次 ATEQ 测试（仅双测）**：第二阶段 NG 时更新记录为 NG 并结束；第二阶段 OK 时更新记录为最终 OK 并进入打印。
4. **正常 OK 打印**：最终 OK 后自动提交当前工位标签打印请求。只有收到打印成功回执，才将周期标记为已打印并开启扫码枪。
5. **正常 OK 扫码与 PLC 放行**：扫码枪只接受当前周期打印标签的二维码；错误码、空码和其他工位的码不会结束流程。扫对后向对应工位发送扫码完成脉冲（A=`M0.1`，B=`M0.0`，2 秒后自动复位），随后关闭扫码枪并归档周期。
6. **正常 NG**：NG 测试只记录结果并结束，不打印标签、不启动扫码枪、不发送扫码完成脉冲。下一周期由该工位独立准备，不影响另一工位。

正常测试的模式点仍为 A=`M0.5`、B=`M0.4`（单测 0、双测 1）。因此，PLC 放行信号的唯一触发条件是“对应标签已经打印成功且扫码正确”，测试结果本身不能提前触发 `M0.1/M0.0`。
